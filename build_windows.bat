...
echo =======================================================
echo    RizkybyMONITOR v1.2 - Windows 1-Click Builder
echo =======================================================
echo.
...
REM Deteksi lokasi dotnet Program Files atau PATH
if exist "%ProgramFiles%\dotnet\dotnet.exe" (
    set "DOTNET_BIN=%ProgramFiles%\dotnet\dotnet.exe"
    set "PATH=%ProgramFiles%\dotnet;!PATH!"
) else (
    where dotnet.exe >nul 2>nul && set "DOTNET_BIN=dotnet.exe"
)

if not "!DOTNET_BIN!"=="" (
    for /f "delims=" %%s in ('"!DOTNET_BIN!" --list-sdks 2^>nul') do set "HAS_DOTNET_SDK=1"
    if not defined HAS_DOTNET_SDK set "DOTNET_BIN="
)
...
if "!DOTNET_BIN!"=="" (
    ...
    if /i "!CONFIRM_DOTNET!"=="y" (
        ...
        if "!PKG_MGR2!"=="winget" (
            echo [INFO] Downloading and installing .NET SDK 8 via winget...
            winget install --id Microsoft.DotNet.SDK.8 -e --force --accept-source-agreements --accept-package-agreements
        ) else if "!PKG_MGR2!"=="choco" (
            echo [INFO] Downloading and installing .NET SDK 8 via chocolatey...
            choco install dotnet-8.0-sdk -y
        ) else if "!PKG_MGR2!"=="scoop" (
            echo [INFO] Downloading and installing .NET SDK via scoop...
            scoop install dotnet-sdk
        )

        REM Cek langsung executable hasil instalasi di Program Files
        if exist "%ProgramFiles%\dotnet\dotnet.exe" (
            set "DOTNET_BIN=%ProgramFiles%\dotnet\dotnet.exe"
            set "PATH=%ProgramFiles%\dotnet;!PATH!"
        ) else (
        ...
        REM Fallback hanya jika package manager benar-benar gagal
        if "!DOTNET_BIN!"=="" (
            echo [INFO] Package manager failed. Installing via official Microsoft .NET script...
            powershell -NoProfile -ExecutionPolicy Bypass -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; &([scriptblock]::Create((Invoke-WebRequest -UseBasicParsing 'https://dot.net/v1/dotnet-install.ps1').Content)) -Channel 8.0 -InstallDir '%ProgramFiles%\dotnet'"
            if exist "%ProgramFiles%\dotnet\dotnet.exe" (
                set "DOTNET_BIN=%ProgramFiles%\dotnet\dotnet.exe"
                set "PATH=%ProgramFiles%\dotnet;!PATH!"
            )
...
if not "!DOTNET_BIN!"=="" (
    if not exist "sensor\rzkmon_sensor.exe" (
        echo [INFO] Building CPU sensor helper - sensor\rzkmon_sensor.exe ...
        echo [INFO] Downloading NuGet packages and compiling - .NET 8 self-contained ...
        if not exist "sensor" mkdir "sensor"
        if not exist "tools\rzkmon_sensor" mkdir "tools\rzkmon_sensor"
        if not exist "tools\rzkmon_sensor\rzkmon_sensor.csproj" (
            (
                ...
                echo   ^</PropertyGroup^>
                echo   ^<ItemGroup^>
                echo     ^<PackageReference Include="LibreHardwareMonitorLib" Version="0.9.4" /^>
                echo   ^</ItemGroup^>
                echo ^</Project^>
            ) > "tools\rzkmon_sensor\rzkmon_sensor.csproj"
        )
        if not exist "tools\rzkmon_sensor\Program.cs" (
            powershell -NoProfile -EncodedCommand 
        )
    ...
    if exist "sensor\rzkmon_sensor.exe" (
        ...
        echo [WARNING] Sensor helper build failed.
    )
)
echo.
